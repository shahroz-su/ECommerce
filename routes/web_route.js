...
router.get("/json1", userController.AllJson1);
router.get("/json2", userController.AllJson2);
...
router.get('/product',userRole, function(req, res){
  Product.find(function(err,docs){
    if (err) throw err;
     Comment1.find(function(err,result){
    if (err) throw err;
    if (result) {
      console.log(result);
...
router.post("/subscribe",userController.Newletter);
